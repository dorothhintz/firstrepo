...
#[test]
fn poc_mul_mod_zero_modulus_is_accepted_by_air() {
    let program_path = get_compiled_cairo_program_path("mul_mod_zero_p");
    ...
    let segment = input
        .builtin_segments
        .mul_mod_builtin
        .expect("mul_mod segment must be present");
    let n_instances = (segment.stop_ptr - segment.begin_addr) / 7;
    assert_eq!(n_instances, 16, "PoC must have exactly 16 real MulMod instances");
    ...
    println!("MUL_MOD_INSTANCES={n_instances}");
    println!("MUL_MODULUS_LIMBS={first_p0},{first_p1},{first_p2},{first_p3}");
    ...
    println!("MUL_FRI_SECURITY_BITS={}", production_fri.security_bits());
    ...
    let proof = prove_cairo::<Blake2sMerkleChannel>(input, prover_params)
        .expect("STWO prover should produce MulMod p=0 proof if AIR permits it");
    verify_cairo::<Blake2sMerkleChannel>(proof.into())
        .expect("unmodified sharp-8.0 verifier accepted MulMod p=0 proof");
    println!("STWO_FULL_STARK_ACCEPTED_MUL_MOD_P_ZERO_AT_96_BITS");
    ...
    match secure_outcome {
        Ok(Ok(_)) => panic!("secure Cairo VM unexpectedly accepted MulMod modulus p=0"),
        Ok(Err(err)) => println!("CAIRO_VM_SECURE_REJECT_MUL_P_ZERO={err:?}"),
        Err(_) => println!("CAIRO_VM_SECURE_PANIC_MUL_P_ZERO"),
    }
    println!("MUL_MOD_ZERO_P_POC_ALL_PASSED");
}
